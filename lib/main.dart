...
import 'package:silicon_labs/view_activity.dart';
                                  ...
                                  if (snapshot.data == BluetoothConnectionState.connected) {
                                    return ElevatedButton(
                                      ...
                                      onPressed: () => Navigator.of(context).push(MaterialPageRoute(
                                          builder: (context) => ViewActivity(device: d), settings: const RouteSettings(name: '/deviceScreen'))),
                                    );
                                  }
                        ...
                        .map(
                          (r) => ScanResultTile(
                            ...
                            onTap: () => Navigator.of(context).push(MaterialPageRoute(
                                builder: (context) {
                                  r.device.connect(timeout: const Duration(seconds: 4)).catchError((e) {
                                    final snackBar = SnackBar(content: Text(prettyException("Connect Error:", e)));
                                    snackBarKeyB.currentState?.showSnackBar(snackBar);
                                  });
                                  return ViewActivity(device: r.device);
                                },
                                settings: const RouteSettings(name: '/deviceScreen'))),
                          ),
                        )
                          ...
                          snapshot.data == BluetoothConnectionState.connected
                              ? StreamBuilder<int>(
                                  ...
                                  builder: (context, snapshot) {
                                    return Text(snapshot.hasData ? '${snapshot.data}dBm' : '', style: Theme.of(context).textTheme.bodySmall);
                                  })
